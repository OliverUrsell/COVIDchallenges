    ...
    <body>
        ...
        <?php
            if($navbarLoggedIn && isset($_POST["distanceTotal"])){
                ...
                $sql = "SELECT `AUTO_INCREMENT` FROM  INFORMATION_SCHEMA.TABLES WHERE TABLE_SCHEMA = '". $conn->real_escape_string($dbname) ."' AND TABLE_NAME = 'tbljourneys';";
                ...
                if ($conn->query($sql) === TRUE) {
                    ...
                    for ($i=0; $i < $_POST["latLongCount"]; $i++) {
                        $sql = "INSERT INTO `tbllatlongs` (`JourneyID`, `CoordinateIndex`, `Latitude`, `Longitude`) VALUES ('". $conn->real_escape_string($journeyID) ."', '". $i ."', '". $conn->real_escape_string($_POST["lat". ($i + 1)]) ."', '". $conn->real_escape_string($_POST["long". ($i + 1)]) ."')";
                        if ($conn->query($sql) === TRUE){
                    ...
                    $URL='../shareStartJourney/shareStartJourney.php?journeyID='.htmlspecialchars($journeyID);
